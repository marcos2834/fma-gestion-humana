...
// PDF generation temporalmente deshabilitada (libnss3 issue en Vercel runtime)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // NOTA: PDF temporalmente deshabilitado por bug de libnss3.so en runtime Vercel.
    // Se va a reintroducir con servicio externo (PDFShift/Browserless) en próxima iteración.
    const baseUrl = process.env.APP_BASE_URL || 'https://fmagestionhumana.com.ar';
    console.log(`Enviando email a ${meta.email} con link ${baseUrl}/informe/${meta.report_id}`);
